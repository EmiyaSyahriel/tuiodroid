...
public class TouchView extends SurfaceView implements SurfaceHolder.Callback {
	 ...
	 * @param {String} oscIP
	 * @param {Int} oscPort
	 * @param {boolean} drawAdditionalInfo
	 * @param {boolean} sendPeriodicUpdates
	 */
	...
	private void drawInfo(Canvas c) {
		...
		if (!oscInterface.isReachable()) {
			c.drawText(getContext().getString(R.string.err_client_refused), 5, height-2*textPaint.getTextSize()-5,textPaint );
		}
		
		String sourceString = getContext().getString(R.string.fmt_tuio_src, sourceName);
	
		c.drawText(sourceString, 5, height-textPaint.getTextSize()-5,textPaint );
		String clientString = getContext().getString(R.string.fmt_tuio_client_addr, this.oscInterface.getInetAdress(), this.oscInterface.getPort()) ;
